...
std::vector<KvrocksIndexEntry> build_indexes(const PreparedDnsEntryRow& row) {
  ...
  if (row.max_bid_address && row.auction_end_time) {
    // Active auction: expose "items where X is the current auction bidder", ordered by auction end time.
    // Once the auction settles the payload is rewritten without max_bid_address, so no bidder entry is
    // emitted and the stale member is evicted by the Lua cleanup.
