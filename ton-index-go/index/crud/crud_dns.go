...
// dnsRecordsToAuctions projects DNS entry rows into auction views, dropping rows whose auction already
// settled between index emission and payload read (the payload is rewritten without max_bid_address /
// auction_end_time once the winner claims, so a lingering index member would otherwise show up empty).
func dnsRecordsToAuctions(records []models.DNSRecord, now int64) []models.DNSAuction {
...
// dnsAuctionMetadataAddressList collects the addresses that carry NFT metadata: every auction's item, plus
// the collection of any attached item (present only when include_nft_items was requested).
...
// attachAuctionNFTItemsPg loads the full NFT item (collection-joined, on-sale enriched) for each auction's
// nft_item_address via the shared nft-items-by-address query machinery and hangs it off the auction.
